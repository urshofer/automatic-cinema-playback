...
//    if (_videoSize.height != CVPixelBufferGetHeight(_latestPixelFrame) || _videoSize.width  != CVPixelBufferGetWidth(_latestPixelFrame)) {
  //      _videoSize.height = CVPixelBufferGetHeight(_latestPixelFrame);
    //    _videoSize.width  = CVPixelBufferGetWidth(_latestPixelFrame);
    ...
    // video.width * video.height * 3, so we use vImage to copy them into the out buffer.
        ...
        // 	5120 × 3200
//        vImage_Buffer dest = { outbuf, 3200, 5120, 5120 * 3 };

        vImage_Buffer dest = { outbuf, CVPixelBufferGetHeight(_latestPixelFrame), CVPixelBufferGetWidth(_latestPixelFrame), CVPixelBufferGetBytesPerRow(_latestPixelFrame) };
